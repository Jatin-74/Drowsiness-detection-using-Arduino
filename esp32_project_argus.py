...
import logging
import numpy as np
import serial  # <-- To talk to Arduino Uno
...
# !! IMPORTANT: Change this IP to the one from your ESP32's Serial Monitor !!
ESP32_IP = ""  # Your specified IP
ESP32_STREAM_URL = f""
# ---

# --- ARDUINO SERIAL CONNECTION ---
LAST_ARDUINO_ALERT = '0' # Start with 'All Clear'
try:
    # Port is set to COM5 as you requested
    arduino = serial.Serial(port='COM5', baudrate=9600, timeout=0.1)
    logger.info("Arduino connected on COM5.")
except serial.SerialException:
    arduino = None
    logger.warning("Arduino not found on COM5. Running in visual-only mode.")
...
# --- DETECTION TIME (LOWERED FOR FASTER RESPONSE) ---
EYE_AR_CONSEC_FRAMES = 10         # Was 20
MOUTH_OPEN_CONSEC_FRAMES = 15     # Was 25
ROLL_CONSEC_FRAMES = 10           # Was 15
PITCH_CONSEC_FRAMES = 10          # Was 15
YAW_CONSEC_FRAMES = 10            # Was 15
MOUTH_COVERED_CONSEC_FRAMES = 10  # Was 15
EYES_COVERED_CONSEC_FRAMES = 10   # Was 15
PHONE_GESTURE_CONSEC_FRAMES = 20  # Was 30
# ---
...
if not cap.isOpened():
    logger.error("Could not open video stream. Check:")
    logger.error(f"1. Is the ESP32-CAM IP address '{ESP32_IP}' correct?")
    logger.error("2. Is the ESP32-CAM connected to the same Wi-Fi as the PC?")
    logger.error("3. Is the ESP32-CAM powered on and running the code?")
    exit()

logger.info("Stream connected! Starting project.")
time.sleep(2.0) # Give the stream a moment to buffer
...
def set_arduino_alert(alert_code):
    """Sends a 1-char alert code to the Arduino if it's new."""
    global LAST_ARDUINO_ALERT
    if arduino and alert_code != LAST_ARDUINO_ALERT:
        try:
            arduino.write(alert_code.encode()) # Send the character as bytes
            LAST_ARDUINO_ALERT = alert_code
            logger.info(f"Sent alert code to Arduino: {alert_code}")
        except Exception as e:
            logger.error(f"Failed to write to Arduino: {e}")
...
while True:
    ret, frame = cap.read()
    if not ret:
        logger.error("Failed to capture frame. Stream may have ended."); break
    ...
    # 4. DETERMINE ALERTS, SEND TO ARDUINO, AND DISPLAY
    
    current_time = time.time() 
    active_alert_code = '0' # Default: All Clear
    y_pos = 30 
    spacing = 30 

    # Check alerts from highest to lowest priority
    if current_time < TAKE_BREAK_DISPLAY_UNTIL:
        active_alert_code = 'B' # 'B' for Break
        cv2.putText(frame, "Please take a break", (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        y_pos += spacing
    
    elif current_time < EYES_CLOSED_DISPLAY_UNTIL:
        active_alert_code = 'E' # 'E' for Eyes
        cv2.putText(frame, "severe drowsiness", (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        y_pos += spacing

    elif current_time < YAWN_DISPLAY_UNTIL:
        active_alert_code = 'Y' # 'Y' for Yawn
        cv2.putText(frame, "yawn detected", (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        y_pos += spacing
    
    elif current_time < PHONE_GESTURE_DISPLAY_UNTIL:
        active_alert_code = 'T' # 'T' for Telephone
        cv2.putText(frame, "Keep the phone down", (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        y_pos += spacing
        
    elif current_time < EYES_COVERED_DISPLAY_UNTIL:
        active_alert_code = 'C' # 'C' for Covered
        cv2.putText(frame, "eyes covered", (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 100, 100), 2)
        y_pos += spacing
        
    elif current_time < MOUTH_COVERED_DISPLAY_UNTIL:
        active_alert_code = 'M' # 'M' for Mouth
        cv2.putText(frame, "mouth covered", (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
        y_pos += spacing
        
    elif current_time < YAW_DISPLAY_UNTIL:
        active_alert_code = 'D' # 'D' for Distraction
        cv2.putText(frame, "avoid distractions", (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 165, 0), 2)
        y_pos += spacing
        
    elif current_time < POSTURE_DISPLAY_UNTIL:
        active_alert_code = 'P' # 'P' for Posture
        cv2.putText(frame, "adjust posture", (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
        y_pos += spacing

    elif current_time < HANDS_OFF_WHEEL_DISPLAY_UNTIL and not specific_hand_alert_active:
        active_alert_code = 'H' # 'H' for Hands
        cv2.putText(frame, "Please keep your hands on the driving wheel", (10, y_pos),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        y_pos += spacing
        
    elif current_time < FACE_MISSING_DISPLAY_UNTIL:
        active_alert_code = 'F' # 'F' for Face
        cv2.putText(frame, "Please look at the camera", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

    # Send the highest priority active alert (or '0' if none are active)
    set_arduino_alert(active_alert_code)
...
hands.close()
if arduino:
    set_arduino_alert('0') # Turn off alerts before closing
    arduino.close()
